...
func TestDelete(t *testing.T) {
	...
	in := httptest.NewRequest(http.MethodDelete, fmt.Sprintf("/api/reference/%s", id.String()), nil)
	...
	handler.HandleDeleteReference(out, in)
	testQuery := db.GetNextTestQuery()
	assert.Equal(t, http.StatusNoContent, out.Code, "status code")
	assert.Equal(t, fmt.Sprintf("delete from references where id = %s;", id), testQuery, "body")
}
