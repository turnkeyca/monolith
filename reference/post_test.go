...
func TestPost(t *testing.T) {
	...
	in := httptest.NewRequest(http.MethodPost, "/api/reference", nil)
	...
	handler.HandlePostReference(out, in)
	testQuery := db.GetNextTestQuery()
	assert.Equal(t, http.StatusNoContent, out.Code, "status code")
	assert.Equal(t, fmt.Sprintf("insert into references (id) values (%s);", id), testQuery, "body")
}
