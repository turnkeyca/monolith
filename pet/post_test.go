...
func TestPost(t *testing.T) {
	...
	in := httptest.NewRequest(http.MethodPost, "/api/pet", nil)
	...
	handler.HandlePostPet(out, in)
	testQuery := db.GetNextTestQuery()
	assert.Equal(t, http.StatusNoContent, out.Code, "status code")
	assert.Equal(t, fmt.Sprintf("insert into pets (id) values (%s);", id), testQuery, "body")
}
