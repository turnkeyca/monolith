...
func TestDelete(t *testing.T) {
	...
	in := httptest.NewRequest(http.MethodDelete, fmt.Sprintf("/api/listing/%s", id.String()), nil)
	...
	handler.HandleDeleteListing(out, in)
	testQuery := db.GetNextTestQuery()
	assert.Equal(t, http.StatusNoContent, out.Code, "status code")
	assert.Equal(t, fmt.Sprintf("delete from listings where id = %s;", id), testQuery, "body")
}
