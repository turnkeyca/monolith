...
func TestDelete(t *testing.T) {
	...
	in := httptest.NewRequest(http.MethodDelete, fmt.Sprintf("/api/user/%s", id.String()), nil)
	...
	handler.HandleDeleteUser(out, in)
	testQuery := db.GetNextTestQuery()
	assert.Equal(t, http.StatusNoContent, out.Code, "status code")
	assert.Equal(t, fmt.Sprintf("delete from users where id = %s;", id), testQuery, "body")
}
